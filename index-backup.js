...
const timeframe = "5m"; // 1-minute timeframe
...
async function analyzeAndTrade() {
  ...
  const trendLong = latestEmaShort > latestEmaLong;
  const pullbackLong = prevPrice > latestEmaShort && latestPrice <= latestEmaShort;
  const breakoutLong = prevPrice <= latestEmaLong && latestPrice > latestEmaLong;

  if (trendLong && (pullbackLong || breakoutLong)) {
    await sendGoogleChatMessage("✅ Strong Long Signal Detected - Entering Long");
    await createMarketOrder("buy");
  }

  const trendShort = latestEmaShort < latestEmaLong;
  const pullbackShort = prevPrice < latestEmaShort && latestPrice >= latestEmaShort;
  const breakoutShort = prevPrice >= latestEmaLong && latestPrice < latestEmaLong;

  if (!trendLong && (pullbackShort || breakoutShort)) {
    await sendGoogleChatMessage("✅ Strong Short Signal - Entering Short position");
...
setInterval(analyzeAndTrade, 5000);
setInterval(pullbackStrategy, 5000);
setInterval(rangeStrategy, 5000);
setInterval(breakoutStrategy, 5000);

// เรียกฟังก์ชัน Trailing Stop Loss ทุกๆ 1 วินาที
setInterval(trailingStopLoss, 1000);